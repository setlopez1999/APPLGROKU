...
| `MULTI_ISP.md` | Runbook para sacar un ISP nuevo (esquema ENV, manifest, imágenes, publicación) |
...
```
brands/<isp>/config.txt  +  brands/<isp>/{logo,icon,intro,login}.png
        │
...
| 2026-09-21 | **Esquema ENV del Kotlin, diseño redondeado y flujo completo** ✅ | **ENV**: se copia el esquema del rediseno tal cual — `config.txt` en formato properties, **mismas claves** (`appname`, `badge`, `accent`, `baseUrl`, `tabHome`…) y mismos nombres de imagen. `brands/<isp>/` es el catálogo y `ENV/` el cliente activo, para poder alternar con un comando. **Formas redondeadas**: SceneGraph no sabe redondear esquinas, así que se genera un **9-patch blanco** por radio y se tiñe con `blendColor` — dos texturas (922 bytes la píldora) cubren todos los colores, que en un Express importa. **Logo**: el del ISP, y si falta NO se dibuja un rectángulo de color: cae al nombre en texto, como el Kotlin. **Flujo**: añadidas Intro, Mi lista y Buscar; las dos últimas reutilizan `ChannelGrid` en vez de duplicar listas. Búsqueda por nombre y número, sin acentos ni mayúsculas, con tests. **393 tests.** | ✅ Verificado en Roku Express: login con logo real, todo redondeado, Mi lista con los 4 favoritos reales |
| 2026-09-21 | **PRIMERA EJECUCIÓN EN HARDWARE REAL (Roku Express)** ✅ | Instalado en un Roku Express a 720p, el hardware objetivo. **4 bugs que el simulador no podía enseñar**: (1) `secureMode` no está en el `Keyboard` sino en su `TextEditBox`, y hasta arreglarlo **la contraseña se veía en claro** (§22); (2) crear `roHttpAgent` en el hilo de render dispara el watchdog y mata la reproducción — el User-Agent va en `content.HttpHeaders` (§23, **cierra la decisión 8.5**); (3) el arranque metía aplanado + parrilla + vídeo en un solo frame y agotaba el presupuesto del hilo de render: ahora va escalonado, un paso por tick (§24); (4) la captura de pantalla no incluye el plano de vídeo, así que un fondo negro no prueba nada (§25). **Verificado en el aparato**: login real, 23 canales en 2 categorías, parrilla con EPG real de 5 columnas, favoritos, sonda de catch-up, registro y cifrado persistentes entre arranques, y **heartbeats con HTTP 200 — que solo se envían si el vídeo está en `playing`**. | ✅ **En Roku Express real** |
